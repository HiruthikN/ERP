...
const RecurringBills = () => {
    ...
    const emptyForm = { profileName: '', vendor: '', vendorName: '', frequency: 'monthly', nextDate: '', endDate: '', notes: '', taxRate: 0 };
    const [form, setForm] = useState(emptyForm);
    const [items, setItems] = useState([{ product: '', quantity: 1, price: '' }]);

    useEffect(() => { fetchData(); fetchVendors(); fetchProducts(); }, []);
    const fetchData = async () => { try { const r = await api.get('/recurring-bills'); setData(r.data.data); } catch (e) { } finally { setLoading(false); } };
    ...
    const handleSubmit = async (e) => { e.preventDefault(); if (!validate()) return; try { await api.post('/recurring-bills', { ...form, items }); closeModal(); fetchData(); } catch (e) { console.error(e); alert(e.response?.data?.message || 'Error'); } };
    const handleToggle = async (id) => { try { await api.put(`/recurring-bills/${id}/toggle`); fetchData(); } catch (e) { } };
    const handleDelete = async (id) => { if (!window.confirm('Delete?')) return; try { await api.delete(`/recurring-bills/${id}`); fetchData(); } catch (e) { } };
    ...
    return (
        <div className="page-container">
            <div className="page-header">
                <div><h1>Recurring Bills</h1><p>Auto-generated vendor bill templates</p></div>
                <button className="btn btn-primary" onClick={() => { setForm(emptyForm); setItems([{ product: '', quantity: 1, price: '' }]); setErrors({}); setShowModal(true); }}><FiPlus /> New Template</button>
            </div>
            <div className="table-container"><table>
                <thead><tr><th>Profile</th><th>Vendor</th><th>Total</th><th>Frequency</th><th>Next Date</th><th>Active</th><th>Actions</th></tr></thead>
                <tbody>{data.map(d => (
                    <tr key={d._id}>
                        <td><strong>{d.profileName}</strong></td>
                        <td>{d.vendor?.name || d.vendorName || '—'}</td>
                        <td>₹{d.total.toLocaleString()}</td>
                        <td><span className="badge badge-info">{d.frequency?.toUpperCase()}</span></td>
                        <td>{d.nextDate ? new Date(d.nextDate).toLocaleDateString() : '—'}</td>
                        <td><button className="btn-icon" onClick={() => handleToggle(d._id)} style={{ color: d.active ? 'var(--success)' : 'var(--text-muted)' }}>{d.active ? <FiToggleRight size={20} /> : <FiToggleLeft size={20} />}</button></td>
            ...
            </table>{data.length === 0 && <div className="empty-state">No recurring bills</div>}</div>

            {showModal && (
                <div className="modal-overlay" onClick={closeModal}><div className={`modal ${formShake ? 'form-shake' : ''}`} onClick={e => e.stopPropagation()}>
                    <div className="modal-header"><h3>New Recurring Bill</h3><button className="btn-icon" onClick={closeModal}><FiX /></button></div>
                    <form onSubmit={handleSubmit} className="modal-form" noValidate>
                        ...
                        <div className="form-row">
                            <div className="form-group"><label>Frequency</label><select value={form.frequency} onChange={e => setForm({ ...form, frequency: e.target.value })}><option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="quarterly">Quarterly</option><option value="yearly">Yearly</option></select></div>
            ...
            {viewing && (
                <div className="modal-overlay" onClick={() => setViewing(null)}><div className="modal" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="modal-form">
                        ...
                        <p><strong>Frequency:</strong> {viewing.frequency}</p>
                        <p><strong>Total:</strong> ₹{viewing.total.toLocaleString()}</p>
                        <p><strong>Active:</strong> {viewing.active ? 'Yes' : 'No'}</p>
